    ...
    def compute_loss(clean_audio, predicted_audio, n_frames_list, device):
        ...
        with torch.no_grad():
            ...
            for n_frames in n_frames_list:
                ...
                masks.append(torch.ones(n_frames, clean_audio.size(1), dtype=torch.float32))

            # [B, T, F] -> [B, F, T]
            binary_mask = pad_sequence(masks, batch_first=True).to(device).permute(0, 2, 1)  

        masked_predicted_audio = predicted_audio * binary_mask
        masked_clean_audio = clean_audio * binary_mask
        
        # CALCOLO MSE LOSS 
        return ((masked_predicted_audio - masked_clean_audio) ** 2).sum() / (binary_mask.sum() + E)
...
def MaskedL1CompressedLoss():
    """
    Ritorna una funzione loss che calcola L1 Error su spettrogrammi compressi,
    ignorando le zone di padding.
    
    Miglioramenti:
    - Usa L1 invece di MSE (spettrogrammi più nitidi).
    - Usa compressione di potenza (0.3) per imitare l'udito umano.
    """
    def compute_loss(clean_audio, predicted_audio, n_frames_list, device):
        """
        Args:
            clean_audio: [B, F, T] (Magnitudo lineare)
            predicted_audio: [B, F, T] (Magnitudo lineare)
            n_frames_list: Lista lunghezze originali
            device: 'cuda' o 'cpu'
        """
        E = 1e-8
        c_factor = 0.3  # Fattore di compressione standard per audio (simula loudness)

        # COMPRESSIONE (Magnitudo -> Magnitudo Compressa)
        clean_compressed = (clean_audio + E).pow(c_factor)
        pred_compressed = (predicted_audio + E).pow(c_factor)

        # CREAZIONE MASCHERA
        if clean_audio.shape[0] == 1:
            return torch.nn.functional.l1_loss(pred_compressed, clean_compressed)
        ...
        masked_pred = pred_compressed * binary_mask
        masked_clean = clean_compressed * binary_mask
        
        # CALCOLO L1 LOSS 
        loss = torch.abs(masked_pred - masked_clean).sum() / (binary_mask.sum() + E)
...
def MaskedL1Loss():
    ...
    def compute_loss(clean_audio, predicted_audio, n_frames_list, device):
        ...
        # CALCOLO L1 LOSS
        return torch.abs(masked_predicted_audio - masked_clean_audio).sum() / (binary_mask.sum() + E)
